...
class FatigueMonitor:
    ...
    def __init__(self):
        ...
        # Overlay message, usually cleared by voice command "dismiss"
        ...
        rospy.loginfo(
            "FatigueMonitor: session topic = %s", session_topic)
        ...
        # Member 6 — head posture node
        if _M6_OK:
            m6_topic = rospy.get_param(
                '~head_posture_topic', '/head_posture_state')
            rospy.Subscriber(
                m6_topic, HeadPosture, self._m6_callback)
            rospy.loginfo(
    ...
    def _m6_callback(self, msg):
        """Receives HeadPosture from Member 6 head_posture_node."""
    ...
    def _session_callback(self, msg):
        ...
        if msg.data and not was_active:
            ...
            rospy.loginfo(
                "[SESSION] Ended — %d minutes elapsed", mins)
    ...
    # ──────────────────────────────────────────────────────────────────────
    # FATIGUE SCORE — integrates M2 and M6 signals
    # ──────────────────────────────────────────────────────────────────────
    def _compute_score(self) -> float:
        ...
        # Head signal: M6 preferred when calibrated
        if self.m6.received and self.m6.calibrated and self.m6.pose_visible:
            head_n = self.m6.posture_score
        else:
            head_n = min(
    ...
    def _draw(self, frame: np.ndarray) -> np.ndarray:
        ...
        if s.active:
            ...
            self._txt(frame, "  (session manager controls)", 60, (80,80,80), 0.40)
        ...
        # Fatigue output
        self._txt(frame, "FATIGUE OUTPUT", 343,
                  (255,255,255), 0.52, True)
        ...
        self._txt(frame, f"  x{self._time_multiplier():.1f} time weight  [{tts_mode}]",
                  410, (80,80,80), 0.38)
        ...
        if now < self.overlay_until and self.overlay_msg:
            ...
            (tw, th), _ = cv2.getTextSize(
                txt, cv2.FONT_HERSHEY_SIMPLEX, 0.58, 2)
            cx, cy, pad = w//2, h-50, 10
            cv2.rectangle(frame,
                          (cx-tw//2-pad, cy-th-pad),
                          (cx+tw//2+pad, cy+pad), (0,0,0), -1)
            cv2.rectangle(frame,
                          (cx-tw//2-pad, cy-th-pad),
                          (cx+tw//2+pad, cy+pad), col, 2)
    ...
    def cleanup(self):
        ...
        rospy.loginfo(
            "FatigueMonitor: log saved to %s", self.log_path)
